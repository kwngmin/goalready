...
function getMonthlyColor(count: number): string {
  if (count === 0)
    return "bg-zinc-100 border-transparent hover:border-zinc-400";
  if (count === 1)
    return "bg-emerald-200 border-emerald-500 hover:border-emerald-600";
  if (count === 2)
    return "bg-emerald-400 border-emerald-700 hover:border-emerald-800";
  return "bg-emerald-600 border-emerald-900 hover:border-emerald-950";
}
    ...
    <div>
      <div className="flex items-center justify-between h-14 border-b border-gray-200">
        <div className="flex items-center gap-1">
          <span className="min-w-18 text-xl font-bold">{year}년</span>
          <button
            type="button"
          ...
          <button
            ...
            disabled={year === currentYear}
            className="ml-1 h-9 rounded-full border px-3 text-sm font-medium text-zinc-500 hover:bg-zinc-100 disabled:opacity-30 disabled:hover:bg-transparent"
          >
            올해
          </button>
        </div>
      ...
      {(() => {
        ...
        const weeklyCellH = Math.max(
          1,
          Math.floor((GRID_HEIGHT - (maxWeeks - 1) * CELL_GAP) / maxWeeks) - 3,
        );
                ...
                >
                  ...
                  {group.weeks.map((week, wi) => (
                    <button
                      ...
                      className={`w-full rounded-xs border transition-transform hover:scale-110 ${getMonthlyColor(week.count)}`}
                      style={{ height: weeklyCellH }}
                    />
                  ))}
                  {Array.from({ length: maxWeeks - group.weeks.length }).map(
                    (_, i) => (
                      <span
                        key={`empty-${i}`}
                        style={{ height: weeklyCellH }}
